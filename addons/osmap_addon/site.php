...
class SppagebuilderAddonOsmap_addon extends SppagebuilderAddons {
    protected  $map_id ='';
    ...
    public function js() {
        ...
        if(isset($this->addon->settings->sp_location_item) && is_array($this->addon->settings->sp_location_item) && $location_count){
            ...
            foreach ($this->addon->settings->sp_location_item as $key => $item) {
                if(isset($item->latitude) && $item->latitude != '' && isset($item->longitude) && $item->longitude != '') {
                    $icon = $item->map_marker.".png";
                    if (isset($item->map_marker) && substr($item->map_marker,0, 4)==='leaf') {
                        $iconAnchor=   '[22, 94]';
                        $popupAnchor=  '[-3, -76]';
                    }
                    elseif (isset($item->map_marker) ){
                        $iconAnchor=   '[12, 40]';
                        $popupAnchor=  '[-3, -30]';

                    }
                    $cordints .=  "['".$item->popup_info."',"
                        . $item->latitude.","
                        .$item->longitude.",'"
                        .$icon."',"
                        .$iconAnchor.","
        ...
        $js = "window.addEventListener('load', function() {   
          ...
          var greenIcon = L.icon({iconUrl: '".JURI::root() ."plugins/sppagebuilder/advancedmaps/addons/assets/images/'+coords[i][3],
            iconAnchor:   coords[i][4],
            popupAnchor:  coords[i][5],
            });
            
            marker = new L.marker([coords[i][1], coords[i][2]], {icon: greenIcon})
